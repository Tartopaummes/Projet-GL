...
public abstract class AbstractExpr extends AbstractInst {
    ...
    /**
     * @return true if the expression is a SSAVariable
     */
    boolean isSSAVariable(){
        return false;
    }

    /**
     * @return true if the expression is a fieldAccess
     */
    ...
    public AbstractExpr verifyRValue(DecacCompiler compiler,
        ...
        if (!(exprType.sameType(expectedType) || (exprType.isClass() && expectedType.isClass() && ((ClassType) exprType).isSubTypeOf((ClassType) expectedType)))) {
            throw new ContextualError("Incompatible types, expected " + expectedType.getName() + " but got " + type.getName() + ". Rule 3.28.", getLocation());
        }
        // If a float is expected, and we are an int, convert the int to float.
        if (expectedType == compiler.environmentType.FLOAT && exprType == compiler.environmentType.INT) {
            AbstractExpr converted = new ConvFloat(this);
